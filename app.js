...
const state = { registers: [], schedule: [], report: [], monthlySchedule: [], reminders: [], calendarDate: null, professionalSearch: [], patients: [], patientDetail: null, sites: [], operationalReport: [], activeRegisterShift: "I", quickMatches: [], documents: { patients: [], selectedIds: new Set(), templates: [], signatures: [], letterheadUrl: "", previewPatient: null } };
...
function renderPatientQuickMatches() {
  ...
  container.innerHTML = matches.map(match => `<button type="button" class="quick-match" data-quick-patient="${match.paciente_id}"><span><strong>${escapeHtml(match.nombre_completo)}</strong><br><small>${match.dni ? `DNI ${escapeHtml(match.dni)}` : "Perfil sin DNI: completa el DNI para registrar"}</small></span><small>${match.turno ? `Turno ${escapeHtml(match.turno)} · ${escapeHtml(match.secuencia)}` : "Sin turno asignado"}</small></button>`).join("");
  document.querySelectorAll("[data-quick-patient]").forEach(button => button.addEventListener("click", () => {
    ...
    setMessage("#registerMessage", match.dni ? `Paciente encontrado: ${match.nombre_completo}. Pulsa Registrar ingreso.` : `Perfil encontrado: ${match.nombre_completo}. Agrega el DNI antes de registrar.`);
    $("#patientLine").focus();
  }));
}
