...
function renderPatientQuickMatches() {
  ...
  document.querySelectorAll("[data-quick-patient]").forEach(button => button.addEventListener("click", () => {
    ...
    $("#fallbackTurn").value = match.turno || "";
    state.quickMatches = [];
    renderPatientQuickMatches();
    setMessage("#registerMessage", `Paciente encontrado: ${match.nombre_completo}. Pulsa Registrar ingreso.`);
    $("#patientLine").focus();
  }));
}
...
async function submitPatient(event) {
  ...
  try {
    ...
    setMessage("#registerMessage", `${data.nombre_completo} · Turno ${data.turno} · Secuencia ${data.secuencia}. ${profileText}${previousText}${data.dni ? ` DNI ${data.dni} copiado al portapapeles.` : " Perfil sin DNI: registra el documento cuando esté disponible."}`);
...
function copyPatientHours() {
  ...
  if (!rows.length) return setMessage("#registerMessage", `No hay pacientes programados en el Turno ${state.activeRegisterShift}.`, true);
  copyText(rows.map(row => row.estado === "activo" || row.estado === "retirado" ? fmtTimeInput(row.registrado_en) : "").join("\n"), `Horas de entrada del Turno ${state.activeRegisterShift} copiadas en el orden clínico programado; faltas y pendientes quedan vacías.`);
...
function scheduleDoctorStarts(rows, doctorTime, licensed = {}) {
  ...
  const arrivals = rows.filter(row => rowArrivalMinutes(row) !== null).sort((a, b) => rowArrivalMinutes(a) - rowArrivalMinutes(b));
  const values = new Map();
  let last = base;
  arrivals.forEach((row, index) => {
    const arrival = rowArrivalMinutes(row);
    let candidate = Math.max(last + (index === 0 ? 3 : 2), arrival + 1);
    const licensedStart = parseClock(licensed[row.paciente_id]?.inicio || "");
    if (licensedStart !== null && candidate === licensedStart) {
      candidate = candidate - 1 > arrival ? candidate - 1 : candidate + 1;
    }
    last = candidate;
    values.set(row.paciente_id, last);
  });
  return values;
}
function scheduleDoctorEnds(rows, licensed, offsets) {
  const reserved = new Set(Object.values(licensed).map(row => parseClock(row?.fin || "")).filter(value => value !== null));
  const byModule = new Map();
  const values = new Map();
  rows.forEach((row, index) => {
    const finalLicensed = parseClock(licensed[row.paciente_id]?.fin || "");
    if (finalLicensed === null) return;
    const module = moduleForRow(row, index);
    let candidate = finalLicensed + Number(offsets[Math.min(3, module - 1)] || 1);
    const previous = byModule.get(module);
    if (previous !== undefined && candidate <= previous) candidate = previous + 1;
    while (reserved.has(candidate)) candidate += 1;
    byModule.set(module, candidate);
    values.set(row.paciente_id, candidate);
  });
  return values;
}
function renderScheduleFormat() {
  ...
  const tableRows = rows.map((row, index) => {
    ...
    return `<tr class="${row.estado === "falta" ? "row-absent" : ""}"><td>${index + 1}</td><td><strong>${escapeHtml(row.nombre_completo)}</strong></td><td>${escapeHtml(row.acceso_vascular || "")}</td><td><span class="module-chip module-${moduleForRow(row, index)}">MÓDULO ${moduleForRow(row, index)}</span></td><td>${entry}</td><td>${starts.has(row.paciente_id) ? formatClock(starts.get(row.paciente_id)) : ""}</td><td><input class="clinical-time-input" data-format-licensed="inicio" data-patient-id="${row.paciente_id}" type="time" value="${escapeHtml(values.inicio || "")}" /></td><td><input class="clinical-time-input" data-format-licensed="medio" data-patient-id="${row.paciente_id}" type="time" value="${escapeHtml(values.medio || "")}" /></td><td><input class="clinical-time-input" data-format-licensed="fin" data-patient-id="${row.paciente_id}" type="time" value="${escapeHtml(values.fin || "")}" /></td><td>${ends.has(row.paciente_id) ? formatClock(ends.get(row.paciente_id)) : ""}</td><td>${row.estado === "falta" ? "FALTÓ" : row.estado === "pendiente" ? "PENDIENTE" : "PRESENTE"}</td></tr>`;
  }).join("");
  $("#registerToolContent").innerHTML = `<section class="clinical-format-form"><div class="format-topbar"><label>Turno<select id="formatShift">${registerShifts.map(item => `<option value="${item}" ${item === shift ? "selected" : ""}>Turno ${item}</option>`).join("")}</select></label><label>Hora de asistencia del médico<input id="formatDoctorEntry" type="time" value="${state.scheduleFormat.doctorEntry || currentDoctorEntryTime()}" /></label><label>Fin Módulo 1 (+ min)<input id="formatEndOffset1" type="number" min="1" max="3" value="${state.scheduleFormat.endOffsets[0]}" /></label><label>Fin Módulo 2 (+ min)<input id="formatEndOffset2" type="number" min="1" max="3" value="${state.scheduleFormat.endOffsets[1]}" /></label><label>Fin Módulo 3 (+ min)<input id="formatEndOffset3" type="number" min="1" max="3" value="${state.scheduleFormat.endOffsets[2]}" /></label></div><p class="register-tool-note">Las faltas y pendientes dejan vacía la hora del paciente. La hora inicial médica sigue las llegadas reales, pero se muestra en el orden clínico fijo. La hora final médica se ajusta para no coincidir con la última columna de licenciados.</p><div class="scroll-table clinical-format-table"><table><thead><tr><th>N°</th><th>Apellidos y nombres</th><th>Acc. V.</th><th>Módulo</th><th>Hora paciente</th><th>Hora inicial médico</th><th>Lic. inicio</th><th>Lic. proceso</th><th>Última lic.</th><th>Hora fin médico</th><th>Estado</th></tr></thead><tbody>${tableRows || '<tr><td colspan="11" class="empty">No hay pacientes programados.</td></tr>'}</tbody></table></div><div class="register-tool-actions"><button class="secondary-btn" data-copy-format="patient" type="button">Copiar hora pacientes</button><button class="secondary-btn" data-copy-format="start" type="button">Copiar inicio médico</button><button class="secondary-btn" data-copy-format="end" type="button">Copiar fin médico</button><button class="primary-btn" data-copy-format="all" type="button">Copiar formato completo</button></div></section>`;
  document.querySelectorAll("[data-format-licensed]").forEach(input => input.addEventListener("input", () => { const id = input.dataset.patientId; state.scheduleFormat.licensed[id] = { ...(state.scheduleFormat.licensed[id] || {}), [input.dataset.formatLicensed]: input.value }; renderScheduleFormat(); }));
  $("#formatShift").addEventListener("change", event => { state.scheduleFormat.shift = event.target.value; state.scheduleFormat.licensed = {}; renderScheduleFormat(); });
  $("#formatDoctorEntry").addEventListener("input", event => { state.scheduleFormat.doctorEntry = event.target.value; renderScheduleFormat(); });
  [1, 2, 3].forEach(index => $(`#formatEndOffset${index}`).addEventListener("input", event => { state.scheduleFormat.endOffsets[index - 1] = Math.min(3, Math.max(1, Number(event.target.value) || 1)); renderScheduleFormat(); }));
...
function copyScheduleFormat(type, rows, starts, ends) {
  ...
  const values = type === "patient" ? rows.map(patient) : type === "start" ? rows.map(start) : type === "end" ? rows.map(end) : rows.map((row, index) => [index + 1, row.nombre_completo, row.acceso_vascular || "", `MÓDULO ${moduleForRow(row, index)}`, patient(row), start(row), licensed(row).inicio || "", licensed(row).medio || "", licensed(row).fin || "", end(row), row.estado === "falta" ? "FALTÓ" : row.estado === "pendiente" ? "PENDIENTE" : "PRESENTE"].join("\t"));
  copyText(values.join("\n"), "Formato clínico copiado para pegar en Excel.");
  setMessage("#registerToolMessage", "Formato clínico copiado para pegar en Excel.");
}
function buildScheduleFormat() {
  state.scheduleFormat = { ...state.scheduleFormat, shift: state.activeRegisterShift, doctorEntry: currentDoctorEntryTime() || state.scheduleFormat.doctorEntry || "", licensed: {} };
