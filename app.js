...
function setServiceIndicator(selector, stateName, text) {
  const element = $(selector);
  if (!element) return;
  element.className = `service-pill ${stateName}`;
  element.innerHTML = `<i></i>${escapeHtml(text)}`;
}

async function checkServiceStatus() {
  const button = $("#checkServices");
  if (button) button.disabled = true;
  setServiceIndicator("#supabaseStatus", "checking", "Supabase: verificando");
  setServiceIndicator("#aiStatus", "checking", "Asistente PDF/IA: verificando");
  const pdfReady = Boolean(window.FileReader && window.fetch && window.Promise);
  try {
    if (!sb) throw new Error("cliente no inicializado");
    const { error } = await sb.rpc("clinic_listar_sedes_atencion");
    if (error) throw error;
    setServiceIndicator("#supabaseStatus", "online", "Supabase: conectado");
  } catch (error) {
    setServiceIndicator("#supabaseStatus", "error", "Supabase: sin conexión");
  }
  setServiceIndicator("#aiStatus", pdfReady ? "online" : "error", pdfReady ? "Asistente PDF/IA: listo" : "Asistente PDF/IA: no disponible");
...
function openRegisterTool(tool) {
  ...
  if (tool === "patient-order-update") return buildPatientOrderUpdate();
}

function buildPatientOrderUpdate() {
  const sequence = sequenceForDate(limaDate()) === "—" ? "LMV" : sequenceForDate(limaDate());
  const shift = state.activeRegisterShift;
  $("#registerToolPanel").hidden = false;
  $("#registerToolTitle").textContent = "Actualizar lista clínica desde Excel";
  $("#registerToolSubtitle").textContent = "Pega una fila por paciente. Se conserva el orden del Excel y se crean perfiles mínimos si no existen.";
  $("#registerToolContent").innerHTML = `<section class="clinical-format-form planning-listbox"><div class="format-topbar format-topbar-simple"><label>Periodo<input id="quickOrderPeriod" type="month" value="${limaDate().slice(0, 7)}" /></label><label>Secuencia<select id="quickOrderSequence"><option value="LMV" ${sequence === "LMV" ? "selected" : ""}>LMV</option><option value="MJS" ${sequence === "MJS" ? "selected" : ""}>MJS</option></select></label><label>Turno<select id="quickOrderShift">${registerShifts.map(item => `<option value="${item}" ${item === shift ? "selected" : ""}>Turno ${item}</option>`).join("")}</select></label></div><label class="patient-order-listbox">Lista de pacientes desde Excel<textarea id="quickPatientOrderPaste" rows="14" placeholder="APELLIDO PATERNO APELLIDO MATERNO NOMBRES 01234567&#10;SIGUIENTE PACIENTE 12345678"></textarea><span>Una fila por paciente. El DNI es opcional. Si el nombre no existe, se crea un perfil mínimo para completar después.</span></label><div class="form-actions"><button class="secondary-btn" id="quickLoadCurrentOrder" type="button">Cargar orden actual</button><button class="primary-btn" id="quickSavePatientOrder" type="button">Procesar y organizar lista</button></div></section>`;
  $("#quickLoadCurrentOrder").addEventListener("click", loadQuickPatientOrder);
  $("#quickSavePatientOrder").addEventListener("click", saveQuickPatientOrder);
  $("#registerToolPanel").scrollIntoView({ behavior: "smooth", block: "start" });
}

function quickOrderParams() {
  return { p_periodo: `${$("#quickOrderPeriod").value || limaDate().slice(0, 7)}-01`, p_secuencia: $("#quickOrderSequence").value, p_turno: $("#quickOrderShift").value };
}

async function loadQuickPatientOrder() {
  const { data, error } = await sb.rpc("clinic_listar_orden_pacientes", quickOrderParams());
  if (error) return setMessage("#registerToolMessage", error.message, true);
  $("#quickPatientOrderPaste").value = (data || []).map(row => `${row.nombre_completo}${row.dni ? `\t${row.dni}` : ""}`).join("\n");
  setMessage("#registerToolMessage", `${(data || []).length} paciente(s) cargados. Puedes reemplazar toda la lista por la que pegues desde Excel.`);
}

async function saveQuickPatientOrder() {
  const p_lista = $("#quickPatientOrderPaste").value.trim();
  if (!p_lista) return setMessage("#registerToolMessage", "Pega la lista de nombres desde Excel antes de procesar.", true);
  const lines = p_lista.replace(/\r/g, "").split("\n").filter(line => line.trim());
  if (lines.length > 15) return setMessage("#registerToolMessage", "Cada turno admite como máximo 15 pacientes.", true);
  if (!confirm(`¿Procesar ${lines.length} paciente(s)? La lista reemplazará el orden del turno seleccionado y se crearán perfiles mínimos para nombres no existentes.`)) return;
  const button = $("#quickSavePatientOrder");
  button.disabled = true;
  const { data, error } = await sb.rpc("clinic_guardar_orden_pacientes_excel", { ...quickOrderParams(), p_lista });
  button.disabled = false;
  if (error) return setMessage("#registerToolMessage", error.message, true);
  setMessage("#registerToolMessage", `Lista organizada: ${data.pacientes_ordenados} pacientes. ${data.pacientes_creados || 0} perfil(es) mínimo(s) creado(s) para completar en Pacientes.`);
  state.activeRegisterShift = data.turno;
  await loadRegisters();
}
...
function renderPatientDetail() {
  ...
  if (!patient) return;
  $("#patientPdfAssistant").hidden = true;
  $("#cancelNewPatient").hidden = true;
  $("#savePatientButton").textContent = "Guardar cambios del paciente";
...
async function savePatientDetail(event) {
  ...
  const { data, error } = id
    ? await sb.rpc("clinic_guardar_paciente", { p_paciente_id: id, p_datos: payload })
    : await sb.rpc("clinic_crear_paciente", { p_datos: payload });
  if (error) return setMessage("#patientDetailMessage", error.message, true);
  state.patientDetail = data;
  await loadPatients();
  renderPatientDetail();
  setMessage("#patientDetailMessage", id ? "Cambios del paciente guardados." : "Nuevo paciente registrado. Revisa y completa los datos que falten.");
}

function resetNewPatientForm() {
  state.patientDetail = null;
  $("#patientDetailForm").reset();
  $("#patientId").value = "";
  $("#patientDetailForm").hidden = false;
  $("#patientPdfAssistant").hidden = false;
  $("#patientDetailTitle").textContent = "Nuevo paciente";
  $("#patientDetailSubtitle").textContent = "Carga una historia clínica en PDF o completa los datos manualmente.";
  $("#patientDocuments").innerHTML = '<p class="empty">El historial de documentos aparecerá después de guardar el perfil.</p>';
  $("#patientWhatsApp").disabled = true;
  $("#savePatientButton").textContent = "Guardar nuevo paciente";
  $("#cancelNewPatient").hidden = false;
  populatePatientSites();
  $("#patientNombres").focus();
}
...
function suggestPatientFromText(text) {
  const fullName = valueAfterLabel(text, "(?:paciente|nombres? y apellidos?|nombre completo)");
  const words = fullName.split(/\s+/).filter(Boolean);
  const dni = (text.match(/(?:DNI|documento|N[°ºo]?\.?\s*doc(?:umento)?)\s*[:\-]?\s*(\d{8})/i) || text.match(/\b(\d{8})\b/))?.[1] || "";
  const age = (text.match(/(?:edad)\s*[:\-]?\s*(\d{1,3})/i) || [])[1] || "";
  const weight = (text.match(/(?:peso\s*seco)\s*[:\-]?\s*(\d{1,3}(?:[.,]\d+)?)/i) || [])[1]?.replace(",", ".") || "";
  const dialysis = (text.match(/(?:tiempo|duraci[oó]n)\s*(?:de\s*)?(?:hemodi[aá]lisis|di[aá]lisis)?\s*[:\-]?\s*(3(?:[.,]5)?)/i) || [])[1]?.replace(",", ".") || "";
  return {
    apellido_paterno: words.slice(0, 1).join(" "), apellido_materno: words.slice(1, 2).join(" "), nombres: words.slice(2).join(" ") || fullName,
    dni, edad: age, peso: weight, diagnostico: valueAfterLabel(text, "(?:diagn[oó]stico|diagn[oó]sticos)"),
    acceso: valueAfterLabel(text, "(?:acceso\s*vascular)"), dialysis,
  };
}

async function extractPatientPdf() {
  const file = $("#patientPdfFile").files[0];
  if (!file) return setMessage("#patientDetailMessage", "Selecciona una historia clínica en PDF.", true);
  try {
    const pdfjs = await import("https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.4.168/pdf.min.mjs");
    pdfjs.GlobalWorkerOptions.workerSrc = "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.4.168/pdf.worker.min.mjs";
    const pdf = await pdfjs.getDocument({ data: await file.arrayBuffer() }).promise;
    const pages = [];
    for (let number = 1; number <= Math.min(pdf.numPages, 12); number += 1) {
    ...
    const suggestion = suggestPatientFromText(text);
    if (suggestion.apellido_paterno) $("#patientApellidoPaterno").value = suggestion.apellido_paterno;
    if (suggestion.apellido_materno) $("#patientApellidoMaterno").value = suggestion.apellido_materno;
    if (suggestion.nombres) $("#patientNombres").value = suggestion.nombres;
    if (suggestion.dni) $("#patientDni").value = suggestion.dni;
    if (suggestion.edad) $("#patientEdad").value = suggestion.edad;
    if (suggestion.peso) $("#patientPesoSeco").value = suggestion.peso;
    if (suggestion.dialysis) $("#patientTiempoDialisis").value = suggestion.dialysis;
    if (suggestion.acceso) $("#patientAccesoVascular").value = suggestion.acceso;
    if (suggestion.diagnostico) $("#patientDiagnostico").value = suggestion.diagnostico;
    const preview = $("#patientPdfTextPreview");
    preview.hidden = false; preview.querySelector("pre").textContent = text.slice(0, 5000);
    setMessage("#patientDetailMessage", "Lectura asistida aplicada. Revisa y corrige cada campo antes de guardar el nuevo paciente.");
  } catch (error) { setMessage("#patientDetailMessage", `No se pudo leer el PDF: ${error.message || "archivo no compatible"}.`, true); }
}
...
function planningPeriodDate() { return `${$("#planningPeriod").value || limaDate().slice(0, 7)}-01`; }

async function loadPlanningHours() {
  const period = planningPeriodDate(), sequence = $("#planningSequence").value, shift = $("#planningShift").value;
  const { data, error } = await sb.rpc("clinic_listar_horarios_turnos_pacientes", { p_periodo: period });
  if (error) return setMessage("#planningEditorMessage", error.message, true);
  const row = (data || []).find(item => item.secuencia === sequence && item.turno === shift);
  $("#planningStartTime").value = String(row?.hora_inicio || "05:30").slice(0, 5);
  $("#planningEndTime").value = String(row?.hora_fin || "09:30").slice(0, 5);
  setMessage("#planningEditorMessage", `Horario ${sequence} · Turno ${shift} cargado. Puedes modificar horas u ordenar pacientes.`);
}

async function loadPatientOrder() {
  const { data, error } = await sb.rpc("clinic_listar_orden_pacientes", { p_periodo: planningPeriodDate(), p_secuencia: $("#planningSequence").value, p_turno: $("#planningShift").value });
  if (error) return setMessage("#planningEditorMessage", error.message, true);
  $("#patientOrderPaste").value = (data || []).map(row => `${row.nombre_completo}${row.dni ? `\t${row.dni}` : ""}`).join("\n");
  setMessage("#planningEditorMessage", `${(data || []).length} paciente(s) cargados en el orden clínico actual.`);
}

async function savePlanningHours() {
  const { error } = await sb.rpc("clinic_guardar_horario_turno_pacientes", {
    p_periodo: planningPeriodDate(), p_secuencia: $("#planningSequence").value, p_turno: $("#planningShift").value,
    p_hora_inicio: $("#planningStartTime").value, p_hora_fin: $("#planningEndTime").value,
  });
  if (error) return setMessage("#planningEditorMessage", error.message, true);
  setMessage("#planningEditorMessage", "Horario del turno guardado. La nueva franja se aplicará al período seleccionado.");
}

async function savePatientOrder() {
  const list = $("#patientOrderPaste").value.trim();
  if (!list) return setMessage("#planningEditorMessage", "Pega la lista de pacientes que viene del Excel.", true);
  if (!confirm("¿Aplicar este orden clínico? Reemplazará las posiciones actuales de la secuencia y turno seleccionados.")) return;
  const { data, error } = await sb.rpc("clinic_guardar_orden_pacientes_excel", {
    p_periodo: planningPeriodDate(), p_secuencia: $("#planningSequence").value, p_turno: $("#planningShift").value, p_lista: list,
  });
  if (error) return setMessage("#planningEditorMessage", error.message, true);
  setMessage("#planningEditorMessage", `Orden actualizado: ${data.pacientes_ordenados} paciente(s). Los módulos se recalcularon cada cinco posiciones.`);
  await loadRegisters();
}
...
function init() {
  ...
  $("#planningPeriod").value = today.slice(0, 7);
  setTheme(localStorage.getItem("clinic-theme") || "light");
  $("#themeToggle").addEventListener("click", () => setTheme(document.documentElement.dataset.theme === "dark" ? "light" : "dark"));
  $("#checkServices").addEventListener("click", checkServiceStatus);
  ...
  $("#refreshRegisters").addEventListener("click", () => openRegisterTool("patient-order-update"));
  ...
  $("#newPatient").addEventListener("click", resetNewPatientForm);
  $("#cancelNewPatient").addEventListener("click", () => { $("#patientDetailForm").hidden = true; $("#patientPdfAssistant").hidden = true; state.patientDetail = null; $("#patientDetailTitle").textContent = "Detalle del paciente"; $("#patientDetailSubtitle").textContent = "Selecciona un paciente de la lista."; });
  $("#extractPatientPdf").addEventListener("click", extractPatientPdf);
  ...
  ["#planningPeriod", "#planningSequence", "#planningShift"].forEach(selector => $(selector).addEventListener("change", loadPlanningHours));
  $("#loadPatientOrder").addEventListener("click", loadPatientOrder);
  $("#savePlanningHours").addEventListener("click", savePlanningHours);
  $("#savePatientOrder").addEventListener("click", savePatientOrder);
  ...
  checkServiceStatus(); loadRegisters(); loadSchedule(); generateMonthlySchedule(); loadOperationalReport(); loadPatientSites(); loadPatients(); loadPlanningHours();
}
init();
