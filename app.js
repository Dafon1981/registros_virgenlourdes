...
const state = { registers: [], schedule: [], report: [], monthlySchedule: [], reminders: [], calendarDate: null, professionalSearch: [], patients: [], patientDetail: null, sites: [], operationalReport: [], documents: { patients: [], selectedIds: new Set(), templates: [], signatures: [], letterheadUrl: "", previewPatient: null } };
...
const shiftHours = { I: "05:30–09:30", II: "09:30–14:00" };
...
function renderShift(shift) {
  const rows = state.registers.filter(row => row.turno === shift).sort((a, b) => new Date(a.registrado_en) - new Date(b.registrado_en));
...
async function loadRegisters() {
  ...
  if (error) return setMessage("#registerMessage", error.message, true);
  state.registers = (data || []).sort((a, b) => new Date(a.registrado_en) - new Date(b.registrado_en));
  renderRegisters();
}
...
async function downloadPatientExcel() {
  const [from, to] = periodRange($("#reportScope").value, $("#reportDate").value);
  const { data, error } = await sb.rpc("clinic_reporte_asistencia_pacientes", { p_desde: from, p_hasta: to });
  if (error) return alert(error.message);
  const rows = (data || []).map(row => ({ Fecha: row.fecha_operativa, Nombre: row.nombre_completo, Profesión: "Paciente", DNI: row.dni, Turno: row.turno || "", Secuencia: row.secuencia || "", Estado: row.estado, "Hora de registro": fmtTime(row.registrado_en), "Hora corregida": row.hora_editada_en ? fmtTime(row.hora_editada_en) : "" }));
