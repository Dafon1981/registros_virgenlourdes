(() => {
  ...
  function renderPreview() {
    ...
    const signatureHtml = config.firma.oculto ? "" : signature?.archivo_url ? `<div class="signature-slot has-signature" data-document-block="firma" style="${blockStyle(config.firma, `height:${pctY(config.firma.alto)}`)}"><img class="doc-signature${docs.activeBlock === "firma" ? " is-selected" : ""}" src="${escapeHtml(signature.archivo_url)}" alt="Firma de ${escapeHtml(signature.nombre_profesional)}" /></div>` : `<div class="signature-slot${docs.activeBlock === "firma" ? " is-selected" : ""}" data-document-block="firma" style="${blockStyle(config.firma, `height:${pctY(config.firma.alto)}`)}"><span>${escapeHtml(resolveBlockText("firma", config.firma, values))}</span></div>`;
  ...
  async function removeSignature(id) {
    const signature = docs.signatures.find(item => Number(item.id) === Number(id));
    if (!signature) return;
    if (!confirm(`¿Retirar la firma de ${signature.nombre_profesional}? Dejará de aparecer para nuevos documentos.`)) return;
    const { error } = await sb.rpc("clinic_eliminar_firma_profesional", { p_firma_id: id });
    if (error) return setMessage("#documentMessage", error.message, true);
    if (byId("#documentSignature").value === String(id)) byId("#documentSignature").value = "";
    setMessage("#documentMessage", `Firma de ${signature.nombre_profesional} retirada de la lista.`);
    await loadSignatures();
  }
  ...
  function bindEvents() {
    ...
    byId("#clearDocumentSignature").addEventListener("click", () => { byId("#documentSignature").value = ""; syncBlockInspector(); renderPreview(); setMessage("#documentMessage", "Firma retirada del documento actual."); });
