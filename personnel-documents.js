(() => {
  ...
  function renderRows() {
    ...
    document.querySelectorAll("[data-contract-person]").forEach(input => input.addEventListener("change", () => updateContractPeriods(input)));
  ...
  async function updateContractPeriods(input) {
    ...
    setMessage("#personnelDocumentMessage", firmado ? `Firma registrada en ${periods.length} mes(es)${horaFirma ? ` a las ${horaFirma}` : ""}.` : `${periods.length} contrato(s) marcados nuevamente como pendientes.`);
    await loadDailyDocuments();
  }
