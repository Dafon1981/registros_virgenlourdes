(() => {
  ...
  function renderRows() {
    ...
    document.querySelectorAll("[data-contract-person]").forEach(input => input.addEventListener("change", () => updateContractPeriods(input, input.checked)));
  ...
  async function updateContractPeriods(input, nextFirmado) {
    ...
    document.querySelectorAll("[data-contract-person]").forEach(control => { control.disabled = true; });
    ...
    let { data, error } = await sb.rpc("clinic_actualizar_documentos_personal_multiperiodo", {
    ...
    const periodDates = periods.map(periodDate);
    const confirmedPeriods = Array.isArray(data) ? data.map(item => String(item.periodo || "").slice(0, 10)) : [];
    const needsFallback = error || !periodDates.every(period => confirmedPeriods.includes(period));
    if (needsFallback) {
      const responses = await Promise.all(periodDates.map(period => sb.rpc("clinic_actualizar_documento_personal", {
        p_personal_id: personalId,
        p_tipo_documento: $("#personnelDocumentType").value,
        p_periodo: period,
        p_firmado: firmado,
      ...
      if (fallbackError) {
        ...
        renderRows();
        return setMessage("#personnelDocumentMessage", `No se guardó la firma: ${fallbackError.message}`, true);
      }
    ...
    const firmadoEn = firmado ? (data?.[0]?.firmado_en || new Date().toISOString()) : null;
    ...
    renderRows();
    const horaFirma = firmado && firmadoEn ? formatTime(firmadoEn) : "";
