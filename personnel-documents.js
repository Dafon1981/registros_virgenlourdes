(() => {
  ...
  function renderRows() {
    ...
    document.querySelectorAll("[data-contract-person]").forEach(input => input.addEventListener("change", async () => {
      input.disabled = true;
      const { error } = await sb.rpc("clinic_actualizar_documento_personal", {
        p_personal_id: Number(input.dataset.contractPerson),
        p_tipo_documento: $("#personnelDocumentType").value,
        p_periodo: periodDate($("#personnelDocumentPeriod").value),
        p_firmado: input.checked,
        p_fecha_referencia: $("#personnelDocumentDate").value,
        p_observacion: null,
      });
      if (error) setMessage("#personnelDocumentMessage", error.message, true);
      else { setMessage("#personnelDocumentMessage", input.checked ? "Firma registrada y vinculada a la jornada seleccionada." : "Firma marcada nuevamente como pendiente."); await loadDailyDocuments(); }
  ...
  async function loadDailyDocuments() {
    ...
    const { data, error } = await sb.rpc("clinic_listar_documentos_personal_dia", { p_fecha: date, p_tipo_documento: $("#personnelDocumentType").value });
